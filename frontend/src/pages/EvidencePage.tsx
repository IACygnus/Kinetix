/**
 * EvidencePage — Standalone page for error evidence and findings.
 * Upload screenshots/logs, per-image AI analysis with Gemini Vision, global analysis.
 */
import { useState, useEffect, useCallback } from 'react';
import { Upload, Trash2, FileText, SearchCheck, Sparkles } from 'lucide-react';
...
const CATEGORIES = [
  { value: 'error_screenshot', label: 'Screenshot de Error' },
  { value: 'log_excerpt', label: 'Extracto de Log' },
...
export default function EvidencePage() {
  ...
  const [selectedCategory, setSelectedCategory] = useState('error_screenshot');
  ...
  const loadAttachments = useCallback(async () => {
    ...
    try {
      const res = await fetch(`${apiBase}/executions/${selectedExecId}/image-analyses?attachment_type=evidence`, { credentials: 'include' });
      if (res.ok) {
        setAttachments(await res.json());
      } else {
        const res2 = await fetch(`${apiBase}/executions/${selectedExecId}/attachments?attachment_type=evidence`, { credentials: 'include' });
  ...
  useEffect(() => {
    ...
    fetch(`${apiBase}/executions/${selectedExecId}/evidence-analysis`, { credentials: 'include' })
  ...
  const handleGenerateAI = async () => {
    ...
    try {
      const res = await fetch(`${apiBase}/executions/${selectedExecId}/evidence-analysis`, {
  ...
  const handleAnalyzeAll = async () => {
    ...
    try {
      const res = await fetch(`${apiBase}/executions/${selectedExecId}/analyze-all-images?attachment_type=evidence`, {
        method: 'POST', credentials: 'include', headers: { 'X-CSRF-Token': getCsrfToken() },
      });
      if (res.ok) {
        const data = await res.json();
        alert(`Analisis completado: ${data.analyzed_count} evidencias analizadas, ${data.failed_count} errores`);
        loadAttachments();
      }
    } catch (err) { console.error(err); }
  ...
  return (
    <div className="w-full p-6">
      <div className="flex items-center gap-3 mb-2">
        <SearchCheck className="w-8 h-8 text-[#f5a623]" />
        <h1 className="text-4xl font-bold text-gray-800">Evidencias y Hallazgos</h1>
      </div>
      <p className="text-lg text-gray-500 mb-6">
        Adjunte screenshots de errores, extractos de logs y notas sobre hallazgos. Genere analisis AI por imagen.
      </p>
      ...
      {!selectedExecId && !listLoading && (
        <div className="bg-amber-50 border-2 border-amber-200 rounded-2xl p-8 text-center">
          ...
          <p className="text-lg text-amber-600">Al seleccionar, apareceran los controles para adjuntar evidencias y generar analisis AI.</p>
        </div>
      )}
      ...
      {selectedExecId && (
        <>
          {/* Step 2: Upload */}
          <div className="bg-white rounded-2xl shadow-lg p-6 border border-gray-200 mb-6">
            <h2 className="text-2xl font-bold text-gray-800 mb-4">2. Adjuntar Evidencias</h2>
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              ...
              <div>
                ...
                <input type="text" value={title} onChange={(e) => setTitle(e.target.value)} placeholder="Ej: Error 500 en Checkout"
                  className="w-full bg-white text-gray-800 rounded-xl px-4 py-3 text-lg border border-gray-300 focus:border-[#f5a623]" />
              </div>
              <div className="flex items-end">
                <label className="flex items-center gap-2 px-6 py-3 bg-[#f5a623] text-[#0a1628] rounded-xl cursor-pointer hover:bg-[#f5a623]/90 text-lg font-bold transition-colors">
                  ...
                  <input type="file" accept="image/png,image/jpeg,image/gif,image/webp,.csv,.txt,.log" multiple onChange={handleUpload} disabled={uploading} className="hidden" />
                </label>
              </div>
            </div>
          </div>

          {/* Step 3: Attachments with per-image AI */}
          {attachments.length > 0 && (
            <div className="bg-white rounded-2xl shadow-lg p-6 border border-gray-200 mb-6">
              <div className="flex items-center justify-between mb-4">
                <h2 className="text-2xl font-bold text-gray-800">Evidencias Adjuntas ({attachments.length})</h2>
                {imageCount > 0 && (
              ...
              <div className="space-y-4">
                {attachments.map(att => (
                  <div key={att.id} className="bg-white rounded-2xl border border-gray-100 shadow-sm p-6 mb-6">
                    ...
                    {!att.file_type?.startsWith('image/') && (
                      <div className="bg-gray-100 rounded-xl p-4 flex items-center gap-2 text-lg text-gray-600"><FileText className="w-5 h-5" /> {att.filename}</div>
                    )}
                  </div>
                ))}
              </div>
          ...
          {attachments.length === 0 && (
            <div className="bg-gray-50 border-2 border-dashed border-gray-300 rounded-2xl p-8 text-center">
              <p className="text-lg text-gray-400">No hay evidencias adjuntas aun. Use el formulario de arriba para adjuntar imagenes o archivos.</p>
            </div>
          )}

          {aiAnalysis && (
            <div className="mt-6 bg-white rounded-2xl shadow-lg p-6 border-l-4 border-orange-500 border border-gray-200">
              <h2 className="text-2xl font-bold text-orange-600 mb-3">Analisis Global de Evidencias</h2>
